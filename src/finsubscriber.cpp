...
FinSubscriber::FinSubscriber() = default;

FinSubscriber::~FinSubscriber()
{
    stop();
    if (d_connected)
    {
...
bool FinSubscriber::connect(const std::string &address)
{
...
void FinSubscriber::start_listening()
{
    if (!d_connected)
    {
...
void FinSubscriber::stop()
{
    d_running = false;
    std::cout << "Subscriber stopped" << std::endl;
}

void FinSubscriber::disconnect()
{
    stop();
    if (d_connected)
    {
